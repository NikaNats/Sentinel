...
The previous runner posted the entire payload as the JSON body and attempted to use plan-level start/result endpoints. The current suite API (verified against `openid/conformance-suite`) is:
...
| Module execution | plan-level `/start` endpoint | `POST /api/runner?test=...&plan=...&variant=...` for each plan module |
| Module result | plan-level `/result` endpoint | `GET /api/info/<runner-id>` until `status=FINISHED` or `INTERRUPTED` |
| HTML report | *(not downloaded)* | `GET /api/plan/exporthtml/<id>` → zip (unpacked into `report/`) |
| Client keys | omitted | ephemeral per-run JWKS generated by the runner; public keys provisioned into Keycloak |

The runner generates two ephemeral client signing JWKS per local run and keeps the private files outside `artifacts/fapi`; only public JWKS are archived and provisioned into Keycloak. To use pre-generated keys instead, set `FAPI_CLIENT_PRIVATE_JWKS` and `FAPI_CLIENT2_PRIVATE_JWKS` in `FAPI_PLAN_CONFIG`, plus `FAPI_CLIENT_JWKS` and `FAPI_CLIENT2_JWKS` for the provisioning hook.

Status values handled: `PASSED`, `FAILED`, `REVIEW`, `ERROR`, `INTERRUPTED`, `COMPLETED` (terminal); module `RUNNING`/`WAITING`/`CREATED` statuses continue polling, up to `FAPI_MAX_POLL`.
...
# 4. Inspect evidence
open artifacts/fapi/report/index.html     # extracted report pack
