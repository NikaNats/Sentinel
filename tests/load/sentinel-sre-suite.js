...
//
// sentinel-sre-suite.js - 2026 SRE Load, Soak & Capacity suite.
//
...
// NOTE: crypto is now a k6 global (webcrypto graduated from
// k6/experimental/webcrypto in k6 >= v1.x); no import required.
...
// importKey is expensive; cache per VU.
const vuKeyCache = {};
...
async function doTransfer(token, nonce) {
  const { entry, cryptoKey } = await keyFor(__VU);
  const proof = await signProof(entry, cryptoKey, 'POST', TRANSFER_URL, nonce);
    ...
    return {
      spike_scenario: {
        ...
        stages: [
          ...
          { duration: '3m', target: div(SPIKE_MAX_RPS) },
          { duration: '10s', target: div(500) },
        ],
      },
    };
