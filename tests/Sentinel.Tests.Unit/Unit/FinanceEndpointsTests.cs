...
public sealed class FinanceEndpointsTests : IClassFixture<FinanceEndpointsTests.LocalTestFactory>
{
    ...
    public async Task ExecuteTransfer_WithValidParamsWithinRarBounds_Returns200Approved()
    {
        ...
        var result = await response.Content.ReadFromJsonAsync<TransferResponse>(SampleJsonContext.Default.TransferResponse);
        result.Should().NotBeNull();
        result!.Status.Should().Be("Approved");
        result.TransactionId.Should().Be("txn-unique-123");
    }

    [Theory(DisplayName = "🔴 Finance: Invalid monetary amount (zero or negative) is blocked and returns 400 Bad Request")]
    [InlineData(0)]
    [InlineData(-100.50)]
    public async Task ExecuteTransfer_WithInvalidAmount_Returns400BadRequest(decimal invalidAmount)
    {
        ...
        var problem = await response.Content.ReadFromJsonAsync<ProblemDetails>(SampleJsonContext.Default.ProblemDetails);
    ...
    [InlineData(0)]
    [InlineData(-100.50)]
    public async Task ExecuteTransfer_WithInvalidAmount_Returns400BadRequest_Fallback(decimal invalidAmount)
    {
        using var client = _factory.CreateClient();
        var requestPayload = new TransferRequest("txn-unique-123", invalidAmount, "USD", "acc-98765");
    ...
    [Theory(DisplayName = "🔴 Finance: Invalid currency code (non-ISO 4217 or contains numbers/length mismatches) is blocked and returns 400")]
    [InlineData("US")]
    [InlineData("USDE")]
    [InlineData("US1")]
    [InlineData("   ")]
    ...
    [Theory(DisplayName = "🔴 Finance: Missing required fields (TransactionId or DestinationAccount) is blocked and returns 400")]
    [InlineData("", "acc-98765", "TransactionId is required")]
    [InlineData("txn-123", "", "DestinationAccount is required")]
    public async Task ExecuteTransfer_WithMissingFields_Returns400BadRequest(string txnId, string account, string expectedError)
    {
        using var client = _factory.CreateClient();
    ...
    [Fact(DisplayName = "🔴 Finance: Transfer exceeding signed RAR bounds (e.g. higher amount than authorized) is blocked with 403")]
    public async Task ExecuteTransfer_ExceedingRarBounds_Returns403Forbidden()
    {
        _factory.RarValidatorMock
            .Setup(x => x.ValidateByType(It.IsAny<AuthorizationDetail[]>(), "urn:sentinel:finance:transfer", It.IsAny<string>()))
    ...
    public sealed class LocalTestFactory : WebApplicationFactory<Program>
    {
        ...
        public void ResetMocks()
        {
            ...
            IdempotencyStoreMock
                .Setup(x => x.MarkCompletedAsync(It.IsAny<string>(), It.IsAny<CachedHttpResponse>(), It.IsAny<TimeSpan>(), It.IsAny<CancellationToken>()))
                .Returns(Task.CompletedTask);
            ...
            builder.Configure(app =>
            {
                ...
                app.Use(async (context, next) =>
                {
                    var claims = new[]
                    {
                        ...
                        new Claim("authorization_details", "[{\"type\":\"urn:sentinel:finance:transfer\",\"transaction_id\":\"txn-unique-123\",\"amount\":50000.00,\"currency\":\"USD\"}]")
                    };
                ...
                {
                    endpoints.MapFinanceEndpoints("v1/finance");
                });
            });
        }
    }
