...
//
// The load suite (sentinel-sre-suite.js) MUST present RFC 9449 proofs signed
// by a real P-256 key whose RFC 7638 thumbprint (JKT) matches the "cnf"."jkt"
// claim inside the bearer JWT, or Sentinel rejects the request at
// DpopProofValidator.cs (missing_cnf_jkt / jkt_mismatch). k6's crypto module
// cannot sign ECDSA, so the P-256 key pairs are provisioned here with Node's
// WebCrypto, and each k6 VU re-signs a fresh proof per request from its own
// private key.
//
// Output: a JSON pool file consumed by the k6 suite:
//   {
//     "generated_at": 1760000000,
//     "issuer": "keycloak" | "self",
//     "token_url": "...",
//     "keys": [
//       { "jwk": {kty,crv,x,y,d}, "jkt": "<rfc7638>", "token": "<JWT|null>", "mint_note": "..." }
//     ]
//   }
// The "d" scalar is embedded so each k6 VU can sign per-request proofs.
// Treat this file as DPoP key material at rest - do not commit real pools.
//
// When --keycloak-url is set, each access token is minted via the password
// grant carrying a live DPoP proof, so the Keycloak-issued JWT carries
// cnf.jkt. If Keycloak is NOT DPoP-enabled (KC_FEATURES=dpop) the minted
// token will lack cnf and the pool is rejected loudly (fail-close).
//
// Usage:
//   node tests/scripts/mint-dpop-pool.mjs \
//     --count 64 --out tests/load/dpop-pool.json \
//     [--keycloak-url http://localhost:8080 --realm sentinel \
//      --client sentinel-gate --username gate-user --password "$KC_GATE_PW"]
...
function rfc7638(jwk) {
  // Canonical member set: {"crv","kty","x","y"}, lexicographic as RFC 7638.
...
function signProof(jwk, { method, url, nonce }) {
  ...
  // CRITICAL: JWT/DPoP (RFC 7515 §3.1, RFC 9449) require IEEE P1363 raw r||s
  // (exactly 64 bytes for P-256). Node's createSign defaults to ASN.1 DER
  // (~70-72 bytes) which Microsoft.IdentityModel (Sentinel's verifier)
...
async function main() {
  ...
  if (cfg.selftest) {
    // Regression guard for the DER vs IEEE P1363 signature flaw: ES256 JWS
    // signatures MUST be exactly 64 bytes (raw r||s). Node defaults to DER.
  ...
  if (cfg.keycloak_url && !cfg.password) throw new Error('--password is required with --keycloak-url');
    ...
    if (cfg.keycloak_url) {
      try {
        ...
        if (!payload.cnf || !payload.cnf.jkt) {
          ...
          bound += 1;
          mintNote = 'keycloak(password-grant, DPoP-bound): cnf.jkt verified';
        }
      } catch (err) {
