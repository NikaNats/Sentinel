...
internal sealed class NotSupportedMlDsaVerifier(ILogger<NotSupportedMlDsaVerifier> logger) : IMlDsaSignatureVerifier
{
    ...
    {
        logger.LogWarning(
            "SECURITY ALERT: ML-DSA (Post-Quantum) signature verification attempted, but the mathematical backend is not yet installed. Request rejected securely (Fail-Closed). Algorithm: {Alg}",
            algorithm);
        return false;
    }
}
