    ...
    IOptions<TwilioOptions> options) : INotificationProvider
{
    ...
    {
        var requestUri = $"https://api.twilio.com/2010-04-01/Accounts/{Uri.EscapeDataString(options.Value.AccountSid)}/Messages.json";
        ...
        var credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes(options.Value.AccountSid + ":" + options.Value.AuthToken));
