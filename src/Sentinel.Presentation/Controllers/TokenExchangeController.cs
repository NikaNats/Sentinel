...
public sealed class TokenExchangeController(ITokenExchangeService tokenExchangeService) : ControllerBase
{
    ...
    public async Task<IActionResult> ExchangeExternalToken([FromBody] TokenExchangeRequest request,
        CancellationToken ct)
    {
        ...
        var result = await tokenExchangeService.ExchangeExternalTokenAsync(request.ExternalToken, request.ProviderName,
            dpopProof, request.CodeVerifier, ct);
