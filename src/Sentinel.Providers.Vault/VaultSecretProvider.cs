...
public sealed class VaultSecretProvider : ISecretProvider, IDisposable
{
    ...
    public async ValueTask<string?> GetSecretAsync(string secretPath, string key,
    ...
    {
        if (!string.IsNullOrEmpty(_vaultToken) && DateTimeOffset.UtcNow < _tokenExpiry)
        {
            return;
        }
        ...
        try
        {
            ...
            _vaultToken = loginRes?.Auth?.ClientToken ??
                          throw new InvalidOperationException("Vault Login response is empty.");
